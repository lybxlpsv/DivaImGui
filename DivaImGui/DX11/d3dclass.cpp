	...
	float screenDepth, float screenNear)
{
	...
	printf("[DivaImGui] SWAPCHAIN=%d,DISPLAY=%d\n", GraphicsClass::SWAPCHAIN_FORMAT, GraphicsClass::DISPLAY_FORMAT);
	...
	//result = adapterOutput->GetDisplayModeList(DXGI_FORMAT_R16G16B16A16_UNORM, DXGI_ENUM_MODES_INTERLACED, &numModes, NULL);
	//if (FAILED(result))
	{
		result = adapterOutput->GetDisplayModeList((DXGI_FORMAT)GraphicsClass::DISPLAY_FORMAT, DXGI_ENUM_MODES_INTERLACED, &numModes, NULL);
		if (FAILED(result))
		{
			return false;
		}
	...
	swapChainDesc.BufferDesc.Format = (DXGI_FORMAT)GraphicsClass::SWAPCHAIN_FORMAT;
	...
	result = m_swapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&dxColorbuffer);
	if (FAILED(result))
	{
		return false;
	}
	...
	result = m_device->CreateRenderTargetView(dxColorbuffer, NULL, &m_renderTargetView);
	if (FAILED(result))
	{
		return false;
	}
	...
	dxDepthBuffer = m_depthStencilBuffer;
...
void D3DClass::EndScene()
{
	...
	if (GraphicsClass::VSYNC_ENABLED)
	{
